# Configuration Portability — Delta

**Spec refs**: `config-portability` (new), `token-import-export`, `custom-token-sets`,
`custom-css-overrides`, `per-app-theming`, `admin-settings`; change `theming-audit-log`
(cross-reference only — a config import is an auditable action there)
**Standards**: OTAP promotion practice (Dutch government environments), JSON, Symfony Console /
Nextcloud occ command registration, nextcloud/server#60173 (upstream `occ theming:config`
shortcomings)
