# Theming Audit Trail — Delta

**Spec refs**: `theming-audit` (new), `admin-settings`, `prometheus-metrics`, `token-sets`,
`custom-token-sets`, `custom-css-overrides`, `theming-sync`, `per-app-theming`; changes
`theme-config-portability` and `theme-preview-workflow` (cross-reference only — their actions
are reserved in the vocabulary here, wiring lands with whichever change merges second)
**Standards**: Besluit digitale toegankelijkheid / Wdo (WCAG-EM evidence expectations), JSON
Lines (ndjson), ISO 8601 timestamps, Nextcloud `IAppDataFactory` appdata storage
