# La Suite Design Stack — Delta

**Spec refs**: `lasuite-stack` (new), `css-architecture` (REQ-CSS-001/005/007/012, ADR-CSS-001/002),
`token-sets` (REQ-TSET-002/008), MinBZK/mijn-bureau (EDIC/sovereign-workplace context),
suitenumerique/cunningham (MIT), suitenumerique/meet#426 (Marianne licensing)
**Standards**: SIL Open Font License 1.1 (Inter), MIT (Cunningham token values), EUPL-1.2
compatibility, WCAG 2.1 AA contrast, CSS Custom Properties
...
`systems/lasuite/defaults`, `systems/lasuite/bridge`, `systems/lasuite/element-overrides`, with
all four files living under `css/systems/lasuite/`. The bundle's goal is a **pixel-adjacent
match to La Suite chrome** — the type ramp, palette, radii, and surface tones of La Suite
numérique apps (Docs/Meet/Chat, Cunningham design system) — so a Nextcloud core inside a
MijnBureau/EDIC sovereign-workplace bundle shows no visual seam against the surrounding La Suite
apps.
...
- AND the four stylesheets MUST be added via `\OCP\Util::addStyle()` in the declared order
...
- AND no `--lasuite-*` custom property MUST be defined

### Requirement: La Suite Fonts Layer With Open Fallback

The fonts layer MUST self-host **Inter** (SIL Open Font License 1.1) as the bundled typeface —
...
`--lasuite-font-family: Marianne, Inter, sans-serif` so Marianne is used ONLY where it is
already installed locally (French administration devices). The app MUST NOT bundle any Marianne
font file: Marianne is restricted to French-state usage and not redistributable under an
MIT/EUPL-compatible license (suitenumerique/meet#426). The OFL license text MUST ship alongside
the Inter files.

#### Scenario: Inter served, Marianne never shipped

- GIVEN the lasuite fonts layer is loaded
- WHEN the browser resolves `--lasuite-font-family` on a machine without Marianne installed
- THEN text MUST render in the bundled Inter
- AND no HTTP request for any Marianne resource MUST occur (no such file exists in the app)

#### Scenario: Locally installed Marianne is honoured

- GIVEN a client device with the Marianne family installed at OS level
- WHEN the font stack resolves
- THEN Marianne MUST be used (first family in the stack, resolved locally)
- AND this MUST require no font bytes from the server
...
- AND no file matching the name Marianne (case-insensitive) MUST exist anywhere in the app

### Requirement: Cunningham Token Defaults Layer

The defaults layer MUST define a curated set of `--lasuite-*` custom properties on `:root`
transcribed from Cunningham (`@gouvfr-lasuite/cunningham-react` token build, MIT), covering at
minimum: the brand scale (`brand-050 #EEF1FA` through `brand-650 #4844AD` to `brand-950
#11131F`), the greyscale (`#FFFFFF` through `#000000` in the steps the bridge consumes),
semantic colors (success `#1E884A`, warning `#CB5000`, error `#E82322`, info `#0077DE`) with
accessible text counterparts, control border radius (`4px`), and a base spacing scale. Each
token group MUST carry a source comment naming the upstream token, and the file header MUST
attribute Cunningham and its MIT license. The layer MUST NOT vendor the full generated
Cunningham token file — only the curated subset the bridge consumes.

#### Scenario: Brand palette matches Cunningham

- GIVEN the defaults layer is loaded
- WHEN `--lasuite-color-brand-650` is resolved
- THEN it MUST equal `#4844AD` (La Suite brand, per `suitenumerique/ui-kit` `cunningham.ts` and
  the shipped Docs `cunningham-tokens.css`)
- AND the semantic colors MUST match the Cunningham 500-level values listed above

#### Scenario: Attribution present

- GIVEN `css/systems/lasuite/defaults.css`
- WHEN its header is read
- THEN it MUST name Cunningham, the MIT license, and the upstream version/commit transcribed from
...
Cunningham token values (MIT), Inter (SIL OFL 1.1). The app MUST NOT bundle Marianne font files,
La Suite or French-state logos, or any other French-government-restricted asset; the lasuite
token set's logo slot MUST remain empty.
