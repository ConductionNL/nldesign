# Spec delta: Token-Set Contrast Audit (shipped-token-set-contrast-audit)

New capability: every shipped token set is verified for WCAG contrast by the same `ContrastService` used for custom uploads, a reproducible report is generated, and non-compliant sets are surfaced instead of silently shipped. This makes the WCAG-AA claims in `GOVERNMENT-FEATURES.md` (F-09, A-01..A-05) true by construction rather than by prose.
