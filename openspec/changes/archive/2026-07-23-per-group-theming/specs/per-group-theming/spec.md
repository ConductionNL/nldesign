# Per-Group Theming — Group → Token-Set Mapping

**Spec refs**: `per-group-theming` (new), `css-architecture` (injection order),
`admin-settings` (mapping UI), `per-app-theming` (orthogonal exclusion), `theming-sync`
(explicitly default-set-only), change `theme-preview-workflow` (preview precedence)
**Standards**: Nextcloud `IGroupManager` / `IUserSession` / `ICacheFactory` APIs; upstream asks
server#23545, server#31749, server#3985 (per-group/tenant theming)
