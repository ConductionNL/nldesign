# Email Theming — New Capability Delta

**Spec refs**: `email-theming` (new), `token-sets` (reads active set + theming metadata),
`theming-sync` (complementary — syncs colors into NC core; this spec brands the mail template
itself), nextcloud/server#2071, server#49620 (toegankelijkheidsverklaring link slot)
**Standards**: `mail_template_class` extension point (`OC\Mail\Mailer::makeTemplate()`,
`lib/private/Mail/Mailer.php:131-143`), Besluit digitale toegankelijkheid / WCAG 2.2 AA
(accessibility statement discoverability), OWASP output-encoding (footer values in HTML mail)
