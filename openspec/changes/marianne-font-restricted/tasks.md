...
- [ ] 1.1 Add `@gouvfr/dsfr@1.15.1` as a **devDependency** in `package.json` (build-only, no
      runtime PHP/JS dependency), consistent with how `@conduction/nextcloud-vue` is used only
      by `scripts/build-icons.js`.
- [ ] 1.2 Copy the DSFR Marianne `woff2` files from `node_modules/@gouvfr/dsfr/dist/fonts/
...
- [ ] 1.3 Verify no file uses `.woff` fallbacks unless needed; prefer `woff2` only for
      CSP-clean self-hosting.

### 2. Gated self-hosted @font-face layer

- [ ] 2.1 Create `css/systems/lasuite/marianne.css` with real `@font-face Marianne`
...
- [ ] 2.2 Leave `css/systems/lasuite/fonts.css` family stack as `--lasuite-font-family:
      ...
      and Inter renders.
- [ ] 2.3 In `lib/Service/CssInjectionService.php::inject()`, after the design-system
...
- [ ] 3.1 Add a settings endpoint on `lib/Controller/SettingsController.php` (routed in
...
- [ ] 3.2 In `lib/Settings/Admin.php`, pass `marianne_enabled` and the active design system id
      into the template parameters.
- [ ] 3.3 In `templates/settings/admin.php`, render — only meaningfully when the selected token
...
- [ ] 3.4 Wire the checkbox in `js/admin.js` to POST the flag and reflect the notice; when
      unchecked, Marianne reverts to Inter on the next render.

### 4. Legal / governance artifacts

- [ ] 4.1 Add `MARIANNE-LICENCE.md` at the repo root carrying the **Etalab Open Licence 2.0**
...
- [ ] 4.2 Add `AGREEMENT-MARIANNE.md` at the repo root: the operator user agreement stating
...
- [ ] 4.3 Add `LICENSES/Etalab-2.0.txt` with the full Etalab Open Licence 2.0 text (REUSE /
      SPDX identifier `Etalab-2.0`).
- [ ] 4.4 Update `.license-overrides.json` to map `css/systems/lasuite/fonts/marianne/*.woff2`
      to `Etalab-2.0` (mirroring how `OFL.txt` accompanies the Inter files).

### 5. Unmissable restriction notice (translatable)

- [ ] 5.1 Add the English source notice string (e.g. *"Marianne is the official typeface of the
...
- [ ] 5.2 Add Dutch translation to `l10n/nl.json` and French translation to `l10n/fr.json`.
- [ ] 5.3 Add a restriction section to `README.md` and to the relevant `docs/` page (font
...
- [ ] 6.1 Unit (PHPUnit, run in the `nextcloud:34` container): assert
...
- [ ] 6.2 Unit: assert `CssInjectionService` emits `systems/lasuite/marianne` when design
      system is `lasuite` AND `marianne_enabled='1'`, and does NOT emit it when the flag is
      `'0'` or the design system is not `lasuite`.
- [ ] 6.3 Unit: assert the `en`/`nl`/`fr` notice keys are present in the respective `l10n`
      files.
- [ ] 6.4 vitest: assert `js/admin.js` posts the flag when the checkbox toggles and shows the
...
- [ ] 6.6 `composer check:strict` passes (PHPCS, PHPMD, Psalm, PHPStan); hydra gates pass
