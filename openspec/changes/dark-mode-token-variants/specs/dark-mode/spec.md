# Dark Mode Token Variants — Delta

**Spec refs**: `dark-mode` (new), `token-sets`, `theming-sync`, `css-architecture` (REQ-CSS-007
dark-compat invariants unchanged), `token-set-contrast-audit`
**Standards**: WCAG 2.1 AA SC 1.4.3 (contrast minimum 4.5:1), CSS Media Queries Level 5
(`prefers-color-scheme`), CSS Custom Properties, NC 34 theming attribute contract
(`body[data-theme-<id>]` / `body[data-themes]`, verified in the server checkout — see change
design.md §1)
