...
- [ ] 1.1 Create `lib/Service/FontValidator.php`: `validate(string $bytes, string $displayName):
...
- [ ] 1.2 Create `lib/Service/FontService.php` using `IAppData` (`fonts/` folder):
...
- [ ] 1.3 Generated stylesheet: `FontService::buildCss(): string` emitting one `@font-face`
...
- [ ] 2.1 Create `lib/Controller/FontController.php`:
...
- [ ] 2.2 `serve(string $id)` (GET): resolve via manifest, return the woff2 bytes with
...
- [ ] 2.3 `css()` (GET): serve `FontService::buildCss()` as `text/css` with the same public
      posture and caching (ETag = font revision); empty 200 stylesheet when no fonts are
      configured.
- [ ] 2.4 Register routes in `appinfo/routes.php`:
...
- [ ] 3.1 In `lib/AppInfo/Application.php` `injectThemeCSS()` path: when the active design
...
- [ ] 4.1 `templates/settings/admin.php`: new "Custom fonts" section — display-name input,
...
- [ ] 4.2 `js/admin.js`: wire upload (FormData POST), list refresh, delete with confirm;
...
- [ ] 4.3 `l10n/` — English source keys + nl translations for all new strings.
- [ ] 4.4 Bump `appinfo/info.xml` `<version>` in the implementation PR so the `?v=` cache
      buster invalidates stale CSS (known stale-bundle gotcha).

## 5. Tests

- [ ] 5.1 `tests/unit/Service/FontValidatorTest.php` — hardening corpus:
...
- [ ] 5.2 `tests/unit/Service/FontServiceTest.php` — store writes `custom-{slug}.woff2` and
...
- [ ] 5.3 `tests/unit/Controller/FontControllerTest.php` — upload maps validator errors to
...
- [ ] 6.1 Run PHPUnit in the nextcloud:34 container and `composer check:strict` — green.
- [ ] 6.2 Live on 8080: upload a real woff2 (e.g. Fira Sans woff2 renamed display-name
      "Test Sans", role body) via the admin panel; confirm
...
- [ ] 6.3 Live in the browser: hard-reload the themed instance, verify via devtools computed
      styles that body text resolves to "Test Sans" and that the login page (logged-out
      window) loads the font without auth errors in the network tab.
- [ ] 6.4 Live negative: `curl -F 'font=@somefile.ttf'` (admin session + CSRF token) →
      422; oversize file → 413; upload as non-admin → rejected by middleware. Delete the
      test font and confirm rendering degrades cleanly back to Fira Sans.
