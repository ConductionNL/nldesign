...
- [x] 1.1 Create `lib/Service/ThemingAuditService.php` (SPDX docblock; promoted
...
- [x] 1.2 Value-summary rules inside the service: scalar old/new values verbatim; CSS payloads
      ...
      content hash.
- [x] 1.3 Rotation: after append, if `audit.jsonl` > 1 MB (1048576 bytes), move it to
      `audit.jsonl.1` (replacing any existing generation) and start a fresh file. No other
      deletion path exists.
- [x] 1.4 Read APIs: `getRecent(int $limit): array` (parsed entries, newest first, from current
...
- [x] 1.5 Failure posture: every filesystem/appdata Throwable inside `log()` is caught, logged
...
- [x] 2.1 `lib/Controller/SettingsController.php` — inject `ThemingAuditService`; wire:
...
- [x] 2.2 `lib/Controller/OverridesController.php` — `setOverrides()` → `overrides_written`
      (old/new = token count + changed keys); `importOverrides()` → `overrides_imported`
      (imported/skipped counts + content hash).
- [x] 2.3 `lib/Controller/CustomTokenSetController.php` — `upload()` → `custom_set_uploaded`
...
- [x] 2.4 Cross-change wiring (guarded — only if the sibling change is already merged when this
      ...
      **Verified absent in this worktree** (no `ConfigBundleService`/`ThemePreviewService` class
      exists) — guard condition satisfied, skipped; `config_imported`/`preview_published` remain
      reserved-but-unwired in `theming-audit`'s vocabulary until whichever sibling change lands.

## 3. Controller, routes, metrics

- [x] 3.1 Create `lib/Controller/AuditController.php`, `#[AuthorizedAdminSetting(Admin::class)]`
...
- [x] 4.1 `templates/settings/admin.php`: add a "Theming audit log" block — table mount point
...
- [x] 4.2 `js/admin.js`: fetch `GET /settings/audit?limit=20` on panel load, render rows
...
- [x] 5.1 `tests/Unit/Service/ThemingAuditServiceTest.php`: entry shape (ts/actor/action/
...
- [x] 5.2 `tests/Unit/Controller/AuditControllerTest.php`: list caps the limit; export headers;
      both admin-annotated.
- [x] 5.3 Extend the call-site controllers' tests: successful `setTokenSet()` logs exactly one
      `token_set_changed` entry with correct old/new; failed validation logs nothing; audit
      service throwing does not change the endpoint's response. Also added dedicated audit tests
      for `OverridesController` (setOverrides/importOverrides) and `CustomTokenSetController`
      (upload/delete) covering all 8 wired call sites.
- [x] 5.4 Extend `MetricsController` tests for `nldesign_audit_entries_total` (HELP/TYPE/value,
      present in the family list). Ran phpunit in the nextcloud:34 container (123 tests, 1403
      assertions, all green) + `composer check:strict` (ALL CHECKS PASSED); SPDX docblocks on
      all new files (hydra gates).

## 6. Verify (live, 8080 dev instance)

(deferred to post-merge live verification — requires the shared 8080 dev instance, done on the
main checkout after this PR merges, per builder-brief ground rules)
