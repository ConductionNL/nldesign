...
test.describe('token-set-contrast-audit', () => {
	...
	test(
		// @e2e openspec/specs/token-set-contrast-audit/spec.md#applying-a-sub-aa-shipped-set-shows-a-warning-but-still-applies
		'Applying a sub-AA shipped set shows a non-blocking contrast warning',
		async ({ page }) => {
			...
			for (const id of subAaCandidates) {
				...
				if (exists > 0) { flagged = id; break }
			}
			expect(flagged, 'Expected a known sub-AA shipped set in the dropdown').toBeTruthy()
			...
			await expect(overlay.locator('.nldesign-dialog-apply, [class*="apply"]').first()).toBeVisible()
	...
	test(
		// @e2e openspec/specs/token-set-contrast-audit/spec.md#a-compliant-shipped-set-shows-no-contrast-warning
		'A compliant shipped set shows no contrast warning',
		async ({ page }) => {
			await page.goto(THEMING_URL)
			await page.waitForSelector('#nldesign-token-set-select', { timeout: 15_000 })
			await dismissThemingSyncDialog(page)
			...
			if (await overlay.isVisible({ timeout: 10_000 }).catch(() => false)) {
				await expect(overlay.locator('.nldesign-contrast-warning')).toHaveCount(0)
				await overlay.locator('.nldesign-dialog-cancel').first().click()
			}
		},
	)
